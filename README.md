...
[![release](https://img.shields.io/github/release/step-security/taiki-e-install-action?style=flat-square&logo=github)](https://github.com/step-security/taiki-e-install-action/releases/latest)
[![github actions](https://img.shields.io/github/actions/workflow/status/step-security/taiki-e-install-action/ci.yml?branch=main&style=flat-square&logo=github)](https://github.com/step-security/taiki-e-install-action/actions)
...
```yaml
- uses: step-security/taiki-e-install-action@cargo-hack
```
...
```yaml
- uses: step-security/taiki-e-install-action@cargo-hack
- uses: step-security/taiki-e-install-action@cargo-minimal-versions
```
...
The default fallback (cargo-binstall) is often affected by GitHub's API rate limits, so we [pass the `${{ github.token }}` to cargo-binstall](https://github.com/step-security/taiki-e-install-action/issues/561). Disabling the cargo-binstall fallback prevent passing token so helps enhance security.
...
When installing with `step-security/taiki-e-install-action@<tool_name>`, `tool: <tool_name>`, or `tool: <tool_name>@<omitted_version>`, The tool version is reflects upstream releases with a delay of one to a few days (as with other common package managers that verify checksums or signatures). A delay of at least one day is known as [dependency cooldown] and is intended to mitigate the risk of supply chain attacks (the specific cooldown period may be changed in the future). You can bypass the cooldown by explicitly specifying a version. If you want a longer cooldown, consider using the property described below.
...
On Linux, if any required tools are missing, this action will attempt to install them from distro's package manager, so no pre-setup is usually required (except for CentOS or Debian 10 (or older) or very old distro described below, which was already EoL and needs to use vault/archive repos -- see "Install requirements" in [our CI config](https://github.com/step-security/taiki-e-install-action/blob/HEAD/.github/workflows/ci.yml) for example of setup).
...
Known environments affected by the above version requirements are CentOS 6 (EoL on 2020-11) using curl 7.19, and Ubuntu 12.04 (EoL on 2017-04) using curl 7.22 (see "Install requirements" in [our CI config](https://github.com/step-security/taiki-e-install-action/blob/HEAD/.github/workflows/ci.yml) for example of workaround).
